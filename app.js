...
const STATS_URL = "event-stats.json"; // optional, generated by scrape_event_stats.py — see README
...
let allEvents = [];
let eventStats = {}; // slug -> stats, from the optional static event-stats.json
let queue = [];
let currentIndex = 0;
...
let quizMode = "daily"; // "daily" | "practice"
...
async function init() {
  ...
  // Optional: static, hand-scraped real event stats (see
  // scrape_event_stats.py). Loaded once, silently skipped if the file
  // doesn't exist yet — nothing breaks either way.
  try {
    const statsRes = await fetch(STATS_URL);
    if (statsRes.ok) eventStats = await statsRes.json();
  } catch (e) { /* file not present yet — fine, stats just won't show */ }
  ...
  document.getElementById("start-daily-btn").addEventListener("click", () => startGame("daily"));
  document.getElementById("start-random-btn").addEventListener("click", () => startGame("practice"));
  ...
  // Game doesn't auto-start — the start screen (visible by default in the
  // HTML) waits for the person to choose Daily Challenge or Random Pack.
}
...
function startGame(mode) {
  ...
  // The map was built while hidden (display:none), so Leaflet's internal
  // size calculations are stale until we force a recheck now it's visible.
...
// photo (sourced from parkrun's own event pages via our event-info
...
// The event's course description (and real stats, if you've run
// scrape_event_stats.py and added event-stats.json) are shown after
// guessing, as a reveal. The photo itself is shown earlier (pre-guess).
